      ...
      <div className="max-w-container-width mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          <nav className="flex flex-col gap-3" aria-label="Footer navigation">
            <p className="text-xs font-semibold uppercase tracking-wider text-text-muted mb-4">Navegación</p>
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-small text-text-muted hover:text-[#00E5FF] transition-colors w-fit"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="flex flex-col">
            <p className="text-xs font-semibold uppercase tracking-wider text-text-muted mb-4">Síguenos</p>
            <div className="flex gap-4">
              {socialLinks.map((link) => (
                <a
                  ...
                  className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-text-muted hover:text-[#00E5FF] hover:border-white/20 transition-colors"
                >
                  {link.icon}
                </a>
              ))}
